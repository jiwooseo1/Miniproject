...
// 		alert(sw);
		...
		return true; // submit하지않기 위해 return false로
...
<jsp:include page="../header.jsp"></jsp:include>
			    		...
			    		<c:forEach var="board" items="${boardList }">
			    			<c:choose>
			    				<c:when test="${board.isDelete == 'N' }">
			    					<tr id='board${board.no }' class='board' onclick="location.href='viewBoard?no=${board.no}'">
					    				<td>${board.no }</td>
					    				<td>
					    					<c:if test="${board.step > 0 }">
							    				   <c:forEach var="i" begin="1" end="${board.step }" varStatus="status">
							    				   		<c:if test="${status.last }">
							    				   			<img src="${contextPath }/resources/images/reply.png" width="20px" style="margin-left: calc(20px * ${i}) ">
							    				   		</c:if>
							    				   </c:forEach>
					    					</c:if>
							    				   	${board.title }</td>
					    				...
					    				<td class="" style="color: gray;">
					    					<c:if test="${board.step > 0 }">
							    				   <c:forEach var="i" begin="1" end="${board.step }">
							    				   		<img src="${contextPath }/resources/images/reply.png" width="30px">
							    				   </c:forEach>
					    					</c:if>
	...
	<div class="btns">
		<button type="button" class="btn btn-success" onclick="location.href='writeBoard'">글쓰기</button>
	</div>
...
<%-- 	<div>${param.pageNo == null }</div> --%>
		...
		<ul class="pagination">
		    ...
		    <c:forEach var="i" begin="${requestScope.pagingInfo.startNumOfCurrentPagingBlock}" 
		    	...
		    	<c:choose>
		    		<c:when test="${pagingInfo.pageNo == i}">
		    			<li class="page-item"><a class="page-link active" href="listAll?pageNo=${i}&searchType=${param.searchType}&searchWord=${param.searchWord}">${i}</a></li>
		    		</c:when>
		    		<c:otherwise>
		    			<li class="page-item"><a class="page-link" href="listAll?pageNo=${i}&searchType=${param.searchType}&searchWord=${param.searchWord}">${i}</a></li>
		    		</c:otherwise>
		    	</c:choose>		
		    </c:forEach>
		   ...
		   <c:choose> 
			    <c:when test="${pagingInfo.pageNo < requestScope.pagingInfo.totalPageCnt }">
			    	<li class="page-item"><a class="page-link" href="listAll?pageNo=${pagingInfo.pageNo + 1}&searchType=${param.searchType}&searchWord=${param.searchWord}">Next</a></li>
			    </c:when>
			    <c:when test="${pagingInfo.pageNo == requestScope.pagingInfo.totalPageCnt}">
	...
	<div class="mb-3 mt-3 searchItem">
		<form action="listAll" method="get" class="searchArea">
			...
			<div>
				<div class="input-group mb-3" style="width: 350px;">
				  <input type="text" class="form-control" placeholder="Search" name="searchWord" id="searchWord">
				  <button class="btn btn-success" type="submit" onclick="return searchValid();">검색</button>
				</div>
			</div>
		</form>
	</div>
